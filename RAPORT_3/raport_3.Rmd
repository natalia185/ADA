...
Korzystając z testu bazującego na ilorazie wiarogodności na poziomie istotności $\alpha=0.05$ otrzymana p-wartość wyniosła `r p_val`. Zatem nie mamy podstaw do oducenia hipotezy o symetrii, która jest równoważna hipotezie o brzegowej jednorodności.
...
tabela <- ftable(W1, W2)
tabela2 <- table(W1, W2) 
tabela2 %>% kable(caption='Tablica dwudzielcza dla nowych zmniennych W1 i W2.') %>% 
  column_spec(1, border_left = TRUE) %>%
...
moc_20 <- as.data.frame(read.csv('data//moc_20.csv'))
moc0_20 <- as.data.frame(read.csv('data//moc0_20.csv'))
...
moc_50 <- as.data.frame(read.csv('data//moc_50.csv'))
moc0_50 <- as.data.frame(read.csv('data//moc0_50.csv'))
...
moc_100 <- as.data.frame(read.csv('data//moc_100.csv'))
moc0_100 <- as.data.frame(read.csv('data//moc0_100.csv'))
...
moc_1000 <- as.data.frame(read.csv('data//moc_1000.csv'))
moc0_1000 <- as.data.frame(read.csv('data//moc0_1000.csv'))
...
cbind(model_a$data, fitted = fitted(model_a)) %>%
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table5") %>% 
  column_spec(1, border_left = TRUE) %>%
...
cbind(model_b$data, fitted = fitted(model_b)) %>%
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table6") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
cbind(model_c$data, fitted = fitted(model_c)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table7") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
cbind(model_d$data, fitted = fitted(model_d)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table8") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
cbind(model_e$data, fitted = fitted(model_e)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table9") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
cbind(model_f$data, fitted = fitted(model_f)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table10") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
p_b <- 1-pchisq(deviance(model_b), df = df.residual(model_b))
cbind(model_b$data, fitted = fitted(model_b)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table12") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
p_c <- 1-pchisq(deviance(model_c), df = df.residual(model_c))
cbind(model_c$data, fitted = fitted(model_c)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table13") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
p_d <- 1-pchisq(deviance(model_d), df = df.residual(model_d))
cbind(model_d$data, fitted = fitted(model_d)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table14") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
p_e <- 1-pchisq(deviance(model_e), df = df.residual(model_e))
cbind(model_e$data, fitted = fitted(model_e)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table16") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
p_f <- 1-pchisq(deviance(model_f), df = df.residual(model_f))
cbind(model_f$data, fitted = fitted(model_f)) %>% 
  kable(caption = "Porównanie wyznaczonych liczności na podstawie modelów z rzeczywistymi licznościami danych", label = "Table17") %>% 
  column_spec(1, border_left = TRUE) %>%
  column_spec(5, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
\begin{table}[h!]
		...
		\caption{P-wartości testów statystycznych}
			\begin{tabular}{|c|c|}
				\hline
				Model & p-wartość \\ \hline
				$[1\:3]$ & `r 1-pchisq(deviance(model_a), df = df.residual(model_a))` \\\hline
				$[13]$ & `r 1-pchisq(deviance(model_b), df = df.residual(model_b))`  \\\hline
				$[1\:2\:3]$ & `r 1-pchisq(deviance(model_c), df = df.residual(model_c))` \\\hline
				$[12\:3]$ & `r 1-pchisq(deviance(model_d), df = df.residual(model_d))` \\\hline
				$[12\:13]$ & `r 1-pchisq(deviance(model_e), df = df.residual(model_e))` \\\hline
				$[1\:23]$ & `r 1-pchisq(deviance(model_f), df = df.residual(model_f))`\\\hline
			\end{tabular}
		\end{center}
		\label{tab:tabela4}
\end{table}
...
```{r}
sum(result1$`fitted(model1)`[result1$S == 1 & result1$W1 == 2])/
  (sum(result1$`fitted(model1)`[result1$S == 1]))
```
...
```{r, echo=FALSE}
m1_1 <- sum(result1$`fitted(model1)`[result1$Wyk == 1 & result1$S == 1])/
  (sum(result1$`fitted(model1)`[result1$Wyk == 1]))
m2 <- sum(result2$`fitted(model2)`[result2$Wyk == 1 & result2$S == 1])/
  (sum(result2$`fitted(model2)`[result2$Wyk == 1]))
emp <- sum(result1$Freq[result1$S == 1 & result1$Wyk == 1])/
  (sum(result1$Freq[result1$Wyk == 1]))
```
...
```{r}
m <- sum(result$`fitted(model)`[result$S == 1 & result$P == 'K'])/
  (sum(result$`fitted(model)`[result$S == 1]))
emp <- sum(result$Freq[result$S == 1 & result$P == 'K'])/
  (sum(result$Freq[result$S == 1]))
```
...
\multicolumn{1}{l|}{} & \multicolumn{1}{c|}{Wartość empiryczna} & \multicolumn{1}{c|}{`r emp`}                &                      &  \\ \cline{2-3}
\multicolumn{1}{l|}{} & \multicolumn{1}{c|}{Model {[}13 23{]}}  & \multicolumn{1}{c|}{`r m`}                & \multicolumn{1}{c}{} &  \\ \cline{2-3}
                      &                                         &                                         &                      & 
\end{tabular}
\label{tab:8}
\end{table}
...
p_2 <- 1-pchisq(test$Deviance[2], df = test$Df[2])
```
Prawdopodobieństwo dla pierwszej z hipotez alternatywnych `r p_1` jest większe niż założony poziom istotności, więc nie mamy podstaw do odrzucenia hipotezy zerowej. Natomiast dla drugiej `r p_2` jest mniejszy niż założony poziom istotności $\alpha$, więc odrzucamy hipotezę zerową.
