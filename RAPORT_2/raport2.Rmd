...
```{r, echo = FALSE}
read.csv('size.csv') %>% kable(col.names = c('n', 'test Fishera', 'test chi-kwadrat Pearsona', 'test ilorazu wiarogodności'), caption = 'Tu będzie coś mądrego.') %>%
...
```{r, echo = FALSE}
read.csv('power.csv') %>% kable(col.names = c('n', 'test Fishera', 'test chi-kwadrat Pearsona', 'test ilorazu wiarogodności'), caption = 'Tu będzie coś mądrego.') %>%
  column_spec(1, border_left = TRUE) %>%
  column_spec(4, border_right = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
